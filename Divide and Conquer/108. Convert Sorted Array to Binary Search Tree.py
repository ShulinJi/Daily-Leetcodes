...
#SECOND ATTEMPT
# O(N) for visiting every single node and O(logn) bc recursing stack requires O(logn)
class Solution:
    def sortedArrayToBST(self, nums: List[int]) -> Optional[TreeNode]:
        ...
        # because we do mid // 2, then it means we pick the mid and the diff between left and right never exceed 1 for each level, which is the meaning of balanced.
        mid = len(nums) // 2
